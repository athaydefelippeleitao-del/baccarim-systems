...
async function main() {
  ...
  console.log('SELECT category result:', JSON.stringify({ data, error }, null, 2));

  // 2. Try upsert with category
  const testRow = {
    id: 'test-cat-check-' + Date.now(),
  ...
  console.log('UPSERT with category result:', JSON.stringify({ data: uData, error: uError }, null, 2));
}
