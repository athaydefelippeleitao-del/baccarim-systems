    ...
    <div className="space-y-8 md:space-y-12 animate-in fade-in slide-in-from-bottom-6 duration-700 pb-12">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-1.5">
          <div className="flex items-center space-x-2 text-baccarim-blue text-[10px] font-black uppercase tracking-[0.3em]">
            <i className="fas fa-file-contract text-[10px]"></i>
            <span>Comercial</span>
          </div>
          <h2 className="text-3xl md:text-4xl font-black text-slate-800 tracking-tight">Planilha de Propostas</h2>
        </div>
        <button 
          onClick={() => setShowAddModal(true)}
          className="w-full sm:w-auto flex items-center justify-center gap-2.5 px-7 py-3.5 bg-baccarim-blue text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-baccarim-blue/90 transition-all shadow-lg shadow-baccarim-blue/20 hover:-translate-y-0.5"
        >
          <i className="fas fa-plus text-[10px]"></i> Nova Proposta
        </button>
      </div>
      ...
      <div className="bg-white rounded-3xl md:rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full text-left min-w-[1300px] border-collapse">
            <thead className="bg-slate-50 text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">
              <tr>
                <th className="px-6 md:px-10 py-5 border-b border-slate-100 w-[20%]">Nº PROPOSTA</th>
                <th className="px-6 md:px-10 py-5 border-b border-slate-100 w-[20%]">EMPRESA</th>
                <th className="px-6 md:px-10 py-5 border-b border-slate-100 text-center w-[15%]">DATA ENVIADA</th>
                <th className="px-6 md:px-10 py-5 border-b border-slate-100 text-center w-[10%]">ACEITE</th>
                <th className="px-6 md:px-10 py-5 border-b border-slate-100 w-[25%]">ETAPAS</th>
                <th className="px-6 md:px-10 py-5 border-b border-slate-100 text-center w-[5%]">PDF</th>
                <th className="px-6 md:px-10 py-5 border-b border-slate-100 text-center w-[5%]">AÇÕES</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 bg-white text-slate-700 text-sm">
              {contracts.map(contract => {
                return (
                  <tr key={contract.id} className="hover:bg-slate-50/60 transition-colors align-top group">
                    <td className="px-6 md:px-10 py-6">
                      <input 
                        value={contract.title}
                        onChange={(e) => onUpdateContract({ ...contract, title: e.target.value })}
                        className="bg-transparent border-b border-transparent focus:border-baccarim-blue hover:border-slate-200 transition-colors font-black text-slate-800 outline-none w-full min-w-[200px] px-2 py-1.5"
                    ...
                    <td className="px-6 md:px-10 py-6">
                      <select
                        ...
                        className="bg-transparent border-b border-transparent focus:border-baccarim-blue hover:border-slate-200 transition-colors text-xs md:text-sm font-bold text-slate-700 outline-none w-full min-w-[220px] px-1 py-1.5 cursor-pointer"
                      >
                        {clients.map(c => <option key={c} value={c} className="bg-white text-slate-700">{c}</option>)}
                      </select>
                    </td>
                    <td className="px-6 md:px-10 py-6 text-center">
                      <input 
                        ...
                        className="bg-transparent border-b border-transparent focus:border-baccarim-blue hover:border-slate-200 transition-colors text-xs md:text-sm text-slate-500 font-bold outline-none cursor-pointer px-2 py-1.5"
                      />
                    </td>
                    <td 
                      className="px-6 md:px-10 py-6 text-center cursor-pointer hover:bg-slate-100/50 transition-colors select-none" 
                    ...
                    <td className="px-6 md:px-10 py-4 min-w-[300px]">
                      <div className="flex flex-col gap-2">
                          ...
                          return (
                            <div key={step.id} className="flex items-center gap-2 group/step">
                              <button 
                                ...
                                className={`w-4 h-4 rounded-full flex items-center justify-center border transition-all shrink-0 ${isCompleted ? 'bg-emerald-500 border-emerald-500 text-white shadow-sm' : 'border-slate-300 text-transparent hover:border-emerald-400'}`}
                              >
                              ...
                              <input 
                                ...
                                className={`flex-1 bg-transparent border-b border-transparent group-hover/step:border-slate-200 focus:border-baccarim-blue outline-none text-[11px] md:text-xs font-bold px-1 py-1 transition-colors ${isCompleted ? 'text-emerald-600 line-through opacity-60' : 'text-slate-600'}`}
                                placeholder="Nome da etapa"
                              />

                              <button 
                                onClick={() => removeStep(contract.id, step.id)}
                                className="w-5 h-5 rounded hover:bg-rose-50 text-transparent group-hover/step:text-slate-300 hover:!text-rose-500 transition-all flex items-center justify-center shrink-0"
                                title="Excluir"
                              >
                        ...
                        <button 
                          ...
                          className="w-fit text-[9px] font-black uppercase tracking-widest text-slate-400 hover:text-baccarim-blue transition-colors mt-1 flex items-center gap-1 hover:bg-blue-50/50 px-2 py-1 rounded-lg"
                        >
                          <i className="fas fa-plus text-[8px]"></i> Adicionar
                        </button>
                      </div>
                    </td>
                    {/* PDF Column */}
                        ...
                        {getPdf(contract) ? (
                          <>
                            <button
                              ...
                              className="w-9 h-9 rounded-xl bg-rose-50 text-rose-500 hover:bg-rose-500 hover:text-white flex items-center justify-center transition-all shadow-sm"
                            ...
                            <a
                              ...
                              className="w-9 h-9 rounded-xl bg-sky-50 text-sky-500 hover:bg-sky-500 hover:text-white flex items-center justify-center transition-all shadow-sm"
                              title="Baixar PDF"
                            >
                            ...
                            <label
                              ...
                              className="w-9 h-9 rounded-xl bg-slate-50 text-slate-400 hover:bg-slate-200 hover:text-slate-600 flex items-center justify-center transition-all shadow-sm cursor-pointer"
                          ...
                          <label
                            ...
                            className="flex items-center gap-2 px-3 py-2 rounded-xl border-2 border-dashed border-slate-200 text-slate-400 hover:border-baccarim-blue hover:text-baccarim-blue transition-all cursor-pointer text-[9px] font-black uppercase tracking-widest"
                            title="Anexar PDF"
                          >
                    ...
                    <td className="px-6 md:px-10 py-6">
                      <div className="flex items-center justify-center pt-1">
                        <button 
                          ...
                          className="w-9 h-9 rounded-xl bg-rose-50 text-rose-400 hover:bg-rose-500 hover:text-white flex items-center justify-center transition-all shadow-sm"
